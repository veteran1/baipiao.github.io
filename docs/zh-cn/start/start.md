...
- 勾选了LX51编译，并在`LX51Misc`中配置了REMOVEUNUSED宏
...
这里我使用STC官方提供的STC8H实验箱进行演示，[点我去官网申请](https://www.stcmcudata.com/)
...
同理，上位机部分也是通过STC-ISP助手安装到Keil中(它和STC的芯片包一同注入到Keil当中)。之后，在Keil里面选择硬件仿真，然后设置仿真COM口和波特率即可。
...
* 在Keil 的debug按钮按下之前，我们需要对MCU进行断电复位(最好是串口下载器也断电，防止其他应用占用这个COM口)，让程序从仿真程序开始执行。

* 保证用户自己的程序，对XRAM的使用不得访问最后768个字节，也不能对这768个字节进行清零初始化( ELL已经对启动文件做了优化，大家不用担心)。

* 使用硬件仿真，会占用P3.0和P3.1两个IO口，要将串口1的工作IO切换到别的复用IO上。

* 仿真使用的通讯波特率(在Keil debug里面设置)，要大于等于115200。

* 断点不易过多。

* 对于STC8，XRAM小于2K的型号，不建议使用硬件仿真，容易跑飞，可以先用资源丰富的型号仿真完毕，再烧录到对应MCU中。

* 仿真过程中，不能对MCU进行复位。

* 仿真过程中，要保证串口稳定(保证供电稳定)。
